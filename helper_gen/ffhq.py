...
import json
import torch
import dnnlib # pip install https://github.com/podgorskiy/dnnlib/releases/download/0.0.1/dnnlib-0.0.1-py3-none-any.whl
import copy
...
def load_ffhq(img_dir="/home/DATA/ITWM/lorenzp/ffhq-dataset/ffhq/images1024x1024", max_nr=70000, size=2000, img_size=1024):

    rnd = torch.randint(1, max_nr, (size,)).numpy()
    load_ffhq = ImageFolderDataset(img_dir)

    images = []
    for nr in rnd:
        img = load_ffhq._load_raw_image(nr)
...
if __name__ == "__main__":
    ...
    img_size = 1024
    img_dir = "/home/DATA/ITWM/lorenzp/ffhq-dataset/ffhq/images1024x1024"
    ...
    count = 70000 * img_size * img_size

    for nr in tqdm(range(70000)):
        img = load_ffhq._load_raw_image(nr)
        ...
        img_scaled =  np.expand_dims(np.float32(img / 255.), axis=0)

        # import pdb; pdb.set_trace()
        inputs = torch.from_numpy(img_scaled)
    ...
    outputdir = "/home/lorenzp/DeepfakeDetection/analysis/ffhq/std_mean_dev"
    output_file = os.path.join(outputdir, "ffhq.json")
